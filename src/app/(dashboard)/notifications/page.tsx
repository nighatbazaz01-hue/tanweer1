...
import {
  ...
  CheckCheck, Filter, Trash2, Eye,
} from "lucide-react";
import { PageHeader } from "@/components/common/PageHeader";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { allNotifications, getNotificationsForRole, type Notification, type NotifType } from "@/lib/mockData/notifications";
...
export default function NotificationsPage() {
  ...
  const roleNotifs = getNotificationsForRole(activeRole);
  const [notifs, setNotifs] = useState<Notification[]>(roleNotifs);
  ...
  const grouped = {
    today: filtered.filter((n) => ["2 min ago", "18 min ago", "35 min ago", "1 hr ago", "2 hrs ago", "3 hrs ago", "5 hrs ago"].some((t) => n.timestamp.includes(t.replace(" ago", "")))),
    yesterday: filtered.filter((n) => n.timestamp === "Yesterday"),
    earlier: filtered.filter((n) => !["2 min ago", "18 min ago", "35 min ago", "1 hr ago", "2 hrs ago", "3 hrs ago", "5 hrs ago", "Yesterday"].some((t) => n.timestamp === t || n.timestamp.startsWith(t.split(" ")[0]))),
  };
    ...
    <div className="space-y-5">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="gap-2 text-xs" onClick={markAllRead} disabled={unread === 0}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: "Urgent", value: notifs.filter((n) => n.priority === "urgent").length, color: "text-amber-600 bg-amber-50", icon: AlertTriangle },
          { label: "Today", value: notifs.filter((n) => n.timestamp.includes("ago") || n.timestamp.includes("hr")).length, color: "text-blue-600 bg-blue-50", icon: Eye },
          { label: "Total", value: notifs.length, color: "text-violet-600 bg-violet-50", icon: CheckCheck },
        ].map((s) => (
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {filterOptions.map((f) => (
          <button
            key={f.id}
            onClick={() => setFilter(f.id as NotifType | "all")}
            className={cn(
          ...
          >
            ...
            {f.id !== "all" && (
              <span className="ml-1 opacity-70">
                ({filtered.filter((n) => n.type === f.id).length})
              </span>
            )}
          </button>
        ))}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          {notif.actor && (
                            <Badge variant="secondary" className="text-[10px]">By {notif.actor}</Badge>
                          )}
                          <Badge className={cn("text-[10px] capitalize", typeColor[notif.type])}>
                            {notif.type}
                          </Badge>
