...
import { subjectMarks, studentProfile } from "@/lib/mockData/student";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Average Score", value: `${avg}%`, icon: BookOpen, color: "blue" },
          ...
          { label: "Subjects Improved", value: `${improved}/${subjectMarks.length}`, icon: TrendingUp, color: "purple" },
        ].map(({ label, value, sub, icon: Icon, color }) => (
          <Card key={label}>
            <CardContent className="p-4 flex items-center gap-3">
              <div className={`rounded-lg bg-${color}-100 p-2 shrink-0`}><Icon className={`h-4 w-4 text-${color}-600`} /></div>
              <div className="min-w-0"><p className="text-xs text-muted-foreground">{label}</p><p className="text-lg font-bold truncate">{value}</p>{sub && <p className="text-xs text-muted-foreground">{sub}</p>}</div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Subject Marks</CardTitle></CardHeader>
        <CardContent>
            ...
            {subjectMarks.map((m) => (
              <div key={m.subject} className="rounded-lg border p-4">
                ...
                <div className="h-2 rounded-full bg-muted overflow-hidden">
                  <div className={cn("h-full rounded-full transition-all", m.marks >= 85 ? "bg-emerald-500" : m.marks >= 70 ? "bg-blue-500" : "bg-amber-500")} style={{ width: `${m.marks}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base flex items-center gap-2"><Star className="h-4 w-4 text-amber-500" />Marks Trend</CardTitle></CardHeader>
        <CardContent>
          <TrendLine
