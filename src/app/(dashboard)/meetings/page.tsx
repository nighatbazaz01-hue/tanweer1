...
import {
  ...
  AlertCircle, Video, ChevronRight,
} from "lucide-react";
...
import { meetings, getMeetingsForRole, type Meeting, type RSVPStatus } from "@/lib/mockData/meetings";
...
export default function MeetingsPage() {
  ...
  const roleMeetings = getMeetingsForRole(activeRole);
            ...
            <div className="flex gap-2">
              ...
              {selected.status === "upcoming" && (
                <Button size="sm" className="gap-2 bg-emerald-600 hover:bg-emerald-700">
                  <CheckCircle className="h-4 w-4" /> RSVP Accepted
                </Button>
              )}
            </div>
          }
            ...
            <Card>
              <CardContent className="p-5 space-y-4">
                <div className="flex items-start gap-3 flex-wrap">
                  <Badge className={cn("capitalize", typeColor[selected.type])}>
                    {typeLabel[selected.type]}
                  </Badge>
                  <Badge className={cn("capitalize border", statusStyle[selected.status])}>
                    {selected.status}
                  </Badge>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>{selected.time} — {selected.endTime}</span>
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>{selected.attendees.length} attendees</span>
                  </div>
                </div>
                ...
                {selected.departments && (
                  <div className="flex items-center gap-2">
                    ...
                    {selected.departments.map((d) => <Badge key={d} variant="secondary" className="text-xs">{d}</Badge>)}
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              <Card>
                ...
                <CardContent className="flex flex-wrap gap-2">
                  {selected.attachments.map((att) => (
                    <div key={att} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-50 border border-blue-100 text-xs">
                      <FileText className="h-3.5 w-3.5 text-blue-600" />
                      <span className="text-blue-700 font-medium">{att}</span>
                      <Button variant="ghost" size="sm" className="h-5 px-1 text-xs text-blue-600">↓</Button>
                    </div>
                  ))}
                </CardContent>
              </Card>
              ...
              <CardContent>
                <div className="grid grid-cols-3 gap-2 mb-4 text-center text-xs">
                  <div className="bg-emerald-50 rounded-lg p-2">
                    <p className="font-bold text-emerald-700 text-lg">{acceptedCount}</p>
                    <p className="text-emerald-600">Accepted</p>
                  </div>
                  <div className="bg-red-50 rounded-lg p-2">
                    <p className="font-bold text-red-600 text-lg">{declinedCount}</p>
                    <p className="text-red-500">Declined</p>
                  </div>
                  <div className="bg-amber-50 rounded-lg p-2">
                    <p className="font-bold text-amber-600 text-lg">{pendingCount}</p>
                    ...
                    return (
                      <div key={i} className="flex items-center gap-2.5">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback className="text-xs bg-blue-100 text-blue-700">{att.avatar}</AvatarFallback>
                        </Avatar>
            ...
            <Card>
              ...
              <CardContent className="flex items-center gap-3">
                <Avatar className="h-10 w-10">
                  <AvatarFallback className="bg-violet-100 text-violet-700 font-bold">{selected.organizer.avatar}</AvatarFallback>
                </Avatar>
                <div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Badge className={cn("text-xs border shrink-0 capitalize", statusStyle[meeting.status])}>
                        {meeting.status}
                      </Badge>
                    </div>
